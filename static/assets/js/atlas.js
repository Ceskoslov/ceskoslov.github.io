(function () {
  ...
  const atlasMotion = {
    get matches() { return reducedMotion.matches || root.dataset.atlasMode === 'quiet'; },
    addEventListener: function (type, listener) {
      addMediaListener(reducedMotion, listener);
      document.addEventListener('atlas-mode-change', listener);
    }
  };
  let initialized = false;

  function initialize() {
    if (initialized || root.dataset.atlasMode === 'quiet') return;
    initialized = true;
    try {
      const atlas = atlasBackground || heroSpecimen ? makeContourAtlas(getSessionSeed()) : null;
      if (atlasBackground && atlas) initPageAtlasBackground(atlasBackground, atlas, atlasMotion);
      if (heroSpecimen && atlas) initHeroSpecimen(heroSpecimen, atlas, atlasMotion);
      initAtlasReveals(atlasMotion);
      initCardSurvey(atlas ? atlas.seed : 1);
    } catch (error) {
      // Decorative rendering must never prevent reading or navigation.
      root.classList.remove('has-atlas-motion');
      root.classList.add('atlas-unavailable');
    }
  }
  document.addEventListener('atlas-mode-change', initialize);
  ...
  function initHeroSpecimen(canvas, atlasData, reducedMotion) {
    ...
    function requestFrame() {
      if (!frameId && !document.hidden && !reducedMotion.matches && renderer.supportsMotion()) frameId = requestAnimationFrame(renderFrame);
    }
    ...
    function syncInteraction() {
      ...
      if (!reducedMotion.matches && renderer.supportsMotion()) {
  ...
  function createAtlasHeatRenderer(atlasData, targetCanvas, options) {
    ...
    if (gl && program) {
      configureProgram();
      buffer.addEventListener('webglcontextlost', function (event) {
        event.preventDefault();
        contextLost = true;
        buffer.style.visibility = 'hidden';
      });
      buffer.addEventListener('webglcontextrestored', function () {
    ...
    buffer.dataset.atlasRenderer = gl ? 'webgl' : 'static';

    function resize(width, height, ratio) {
      displayWidth = Math.max(width, 1);
      displayHeight = Math.max(height, 1);
      // Cap both DPR and total work; CPU fallback is a static, low-resolution field.
      const maxPixels = gl ? 2000000 : 180000;
      pixelRatio = Math.min(ratio || window.devicePixelRatio || 1, gl ? 1.5 : 1,
        Math.sqrt(maxPixels / (displayWidth * displayHeight)));
    ...
    function render(focusX, focusY, energy, phase, scrollX, scrollY, contourDensity) {
      lastRender = [focusX, focusY, energy, phase, scrollX, scrollY, contourDensity];
      if (contextLost || root.dataset.atlasMode === 'quiet') return;
      if (gl) {
        gl.useProgram(program);
      ...
      if (fallbackDirty) {
        renderCanvasFallback(displayWidth / 2, displayHeight / 2, 0, 0, 0, 0, contourDensity || 24);
        fallbackDirty = false;
      }
    }
    ...
    return {
      canvas: buffer,
      supportsMotion: function () { return Boolean(gl) && !contextLost; },
      resize: resize,
